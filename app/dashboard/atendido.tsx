...
import { fetchAtendidos } from '../../firebase/saveConsulta' // Ajuste o caminho

interface AtendidoPaciente {
  id: string
  nome: string
  email: string
  telefone: string
...
export default function Atendidos() {
  const [atendidos, setAtendidos] = useState<AtendidoPaciente[]>([])
  const [loading, setLoading] = useState<boolean>(true)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    const loadAtendidos = async () => {
      try {
        const pacientesAtendidos = await fetchAtendidos()
        setAtendidos(pacientesAtendidos)
        setLoading(false)
      } catch (err) {
        console.error('Erro ao carregar pacientes atendidos', err)
        setError('Erro ao carregar pacientes atendidos. Tente novamente mais tarde.')
        setLoading(false)
      }
    }
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-semibold text-green-600 mb-6">Pacientes Atendidos</h1>
        ...
        {loading ? (
          <div className="text-center text-gray-500">Carregando pacientes atendidos...</div>
        ) : (
          <>
            {atendidos.length === 0 ? (
              <div className="text-center text-gray-500">Nenhum paciente atendido encontrado.</div>
            ) : (
